...
class ClusteringMetrics(Metric):
    ...
    def compute(self, exclude_cls=None, pred_to_gt=None):
        metric, pred_to_gt = self.metric_fn(np.array(self.pred_labels), np.array(self.gt_labels), self.n_videos,
                                            exclude_cls, pred_to_gt)
...
def indep_eval_metrics(pred_labels_batch, gt_labels_batch, mask, metrics=['mof', 'f1', 'miou'], exclude_cls=None,
                       pred_to_gt=None):
    """
    ...
    for b in range(B):
        ...
        for metric in metrics:
            ...
            score, p2gt_local = eval_fn(pred_labels_batch[b][mask[b]].cpu().numpy(),
                                        gt_labels_batch[b][mask[b]].cpu().numpy(), 1, exclude_cls, p2gt_local)
